...
def toggle_topic_status(topic_id):
    ...
    # Update the topic status in the database
    result = topics_col.update_one({"id": topic_id}, {"$set": {"Status": data['Status']}})
    if result.matched_count == 0:
        return jsonify({"error": "Topic not found"}), 404
